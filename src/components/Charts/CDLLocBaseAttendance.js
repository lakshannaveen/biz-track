...
//   ReceiptLong, AccessTime, InfoOutlined, EventNote,
...
//   //const [selectedYear, setSelectedYear] = useState( (parseInt(currentYear || new Date().getFullYear(), 10) - 1).toString());
...
//       setTabValue(0);
//       setImageError(false);
//       // setSelectedYear(currentYear || new Date().getFullYear().toString());
//       setSelectedYear(
//       currentYear
...
//   const handleDivisionSelect = useCallback((div) => { setSelectedDivision(div); setExpandedRow(null); }, []);
//   const handleBack           = useCallback(() => { setSelectedDivision(null); setExpandedRow(null); }, []);
...
//   const divisions = [...new Set(data.map((d) => d.division).filter(Boolean))].sort();
//   const locationGroups = {};
//   if (selectedDivision) {
//     data.filter((d) => d.division === selectedDivision).forEach((item) => {
...
//   const divEmployees   = selectedDivision ? data.filter((d) => d.division === selectedDivision) : [];
...
//             {selectedDivision
//               ? `${totalLocations} locations · ${divEmployees.length} employees · ${totalPresent} present`
//               : `${divisions.length} divisions · ${data.length} employees total`}
//           </Typography>
//         </Box>

//         {!selectedDivision && <DivisionStep divisions={divisions} data={data} onSelect={handleDivisionSelect} />}
...
//             {isMobile ? (
//               <Box ref={scrollRef}
//                 sx={{ maxHeight: "calc(100vh - 260px)", overflowY: "auto", overflowX: "hidden", pr: 0.5,
//                   "&::-webkit-scrollbar": { width: 4 }, "&::-webkit-scrollbar-track": { bgcolor: "transparent" }, "&::-webkit-scrollbar-thumb": { bgcolor: "#cbd5e1", borderRadius: 4 } }}>
//                 {Object.entries(locationGroups).map(([location, employees]) => (
//                   <MobileLocationPanel key={location} location={location} employees={employees}
//                     isExpanded={expandedRow === location} onToggle={handleToggle} onViewAll={handleEmployeeClick} />
//                 ))}
//                 <Box sx={{ height: 80 }} />
//               </Box>
//             ) : (
//               <DesktopLocationTable locationGroups={locationGroups} expandedRow={expandedRow}
//                 onExpand={handleToggle} onViewDetails={handleViewDetails} onEmployeeClick={handleEmployeeClick} />
//             )}
//           </>
//         )}
//       </Paper>
