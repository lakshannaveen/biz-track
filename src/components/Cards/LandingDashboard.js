...
const Dashboard = () => {
  ...
  // Simple Post card for frontend-only posts/announcements
  const PostCard = ({ post }) => (
    <Card
      elevation={0}
      sx={{ borderRadius: 3, border: "1px solid #eee", mb: 1 }}
    >
      <CardContent sx={{ p: 2 }}>
        <Box display="flex" alignItems="flex-start" gap={2}>
          <Avatar sx={{ bgcolor: colors.primary }}>{post.author[0]}</Avatar>
          <Box sx={{ flex: 1 }}>
            ...
            <Box display="flex" gap={1} mt={1} alignItems="center">
              <Chip size="small" label={post.tag} />
  ...
  const ModuleTile = ({ title, value, icon, color }) => (
    <Paper
      elevation={0}
      sx={{ p: 1.25, borderRadius: 2, display: "flex", alignItems: "center" }}
    >
      <Box sx={{ bgcolor: `${color}22`, borderRadius: 1, p: 1, mr: 1 }}>
        {icon}
      </Box>
      <Box>
        <Typography variant="caption" color="textSecondary">
      ...
      <List dense>
        {items.map((it, idx) => (
          <ListItem key={idx} disableGutters>
            <ListItemAvatar>
              <Avatar sx={{ bgcolor: colors.info }}>{it.iconChar || it.source[0]}</Avatar>
            </ListItemAvatar>
            <ListItemText
              primary={it.title}
              secondary={it.time}
  ...
  return (
    <Box sx={{ p: 2, backgroundColor: "#f5f5f5", minHeight: "100vh" }}>
      ...
      {/* Module / Universal Summary + Activity + Posts */}
      <Grid container spacing={2} sx={{ mt: 3 }}>
        <Grid item xs={12} md={4}>
          <Paper elevation={0} sx={{ p: 2, borderRadius: 3, height: "100%" }}>
            <Typography variant="h6" sx={{ fontWeight: 600, mb: 2 }}>
              Module Summary
            </Typography>
            <Grid container spacing={1}>
              <Grid item xs={12} sx={{ mb: 1 }}>
                <ModuleTile
                  title="Attendance"
                  value="96%"
                  icon={<People sx={{ color: colors.success }} />}
                  color={colors.success}
                />
              </Grid>
              <Grid item xs={12} sx={{ mb: 1 }}>
                <ModuleTile
                  title="Reservations"
                  value="24"
                  icon={<EventAvailable sx={{ color: colors.primary }} />}
                  color={colors.primary}
                />
              </Grid>
              <Grid item xs={12} sx={{ mb: 1 }}>
                <ModuleTile
                  title="QR Scans"
                  value="1.2k"
                  icon={<QrCodeScanner sx={{ color: colors.info }} />}
                  color={colors.info}
                />
              </Grid>
              <Grid item xs={12}>
                <ModuleTile
                  title="Outstanding Tools"
                  value="6"
                  icon={<Build sx={{ color: colors.warning }} />}
                  color={colors.warning}
                />
              </Grid>
            </Grid>
            <Box sx={{ mt: 2 }}>
              ...
              <Box display="flex" gap={1} mt={1} flexWrap="wrap">
                <Chip label="Add Leave" color="primary" />
                <Chip label="Scan QR" color="info" />
                <Chip label="New Reservation" color="success" />
                <Chip label="Create Ticket" color="warning" />
              </Box>
            </Box>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          <RecentActivity
            items={[
              { title: "Leave approved for A. Smith", time: "10m ago", source: "Leave" },
              { title: "New reservation created", time: "1h ago", source: "Reservation" },
              { title: "Tool returned - Drill", time: "Yesterday", source: "Tools" },
              { title: "New phone directory entry added", time: "2 days", source: "Phone" },
            ]}
          />
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={0} sx={{ p: 2, borderRadius: 3 }}>
            ...
            <Divider sx={{ mb: 2 }} />
            {/* Example static posts shown on dashboard (frontend-only) */}
            {[
              {
                author: "HR Team",
                time: "2 hours ago",
                message:
                  "Company townhall next Friday 10am. Agenda: Q1 results and roadmap.",
                tag: "Announcement",
                comments: 12,
              },
              {
                ...
                time: "1 day ago",
                message:
                  "Scheduled maintenance this weekend. Some services may be unavailable.",
                tag: "Maintenance",
                comments: 3,
              },
              {
                ...
                time: "3 days ago",
                message:
                  "Reminder: update your emergency contact and personal details.",
                tag: "Reminder",
                comments: 5,
              },
            ].map((p, i) => (
